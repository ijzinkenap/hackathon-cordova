...
(function(global){
    ...
    var win = global,
        ...
        _flxhr_css = null,
        _flxhr = null,
        flXHR_js = "flSOAP.js",					// SHOULD NOT rename the file or change this line
        flensed_js = "flensed.js",				// ditto
        flXHR_vbs = "flXHR.vbs",				// ditto
        checkplayer_js = "checkplayer.js",		// ditto
        flXHR_swf = "proxy.swf",				// ditto
    ...
    _flensed.flXHR = function(configObject) {
        ...
        function setParam() {
            updateFromPublicAPI();
            if (typeof arguments[0]===UNDEF||typeof arguments[1]===UNDEF) { doError(_flxhr.CALL_ERROR,"flXHR::setParam(): Failed","The setParam() call requires 'name' and 'value' parameters."); }
            else if (!_error) {	// the SWF will actually catch if the open() hasn't yet been called (ie, readyState = 0), or if the send has already been called
                var a0=(typeof arguments[0]!==UNDEF)?arguments[0]:EMPTY,a1=(typeof arguments[1]!==UNDEF)?arguments[1]:EMPTY;
                try { proxyObj.setParam(a0,a1); }
                catch (err) { doError(_flxhr.CALL_ERROR,"flXHR::setParam(): Failed","The setParam() call failed to complete.");}
            }
        }

        function addFile() {
            updateFromPublicAPI();
            if (typeof arguments[0]===UNDEF||typeof arguments[1]===UNDEF) { doError(_flxhr.CALL_ERROR,"flXHR::addFile(): Failed","The addFile() call requires 'name' and 'value' parameters."); }
            else if (!_error) {	// the SWF will actually catch if the open() hasn't yet been called (ie, readyState = 0), or if the send has already been called
                var a0=(typeof arguments[0]!==UNDEF)?arguments[0]:EMPTY,a1=(typeof arguments[1]!==UNDEF)?arguments[1]:EMPTY,a2=(typeof arguments[2]!==UNDEF)?arguments[2]:EMPTY;
                try { proxyObj.addFile(a0,a1,a2); }
                catch (err) { doError(_flxhr.CALL_ERROR,"flXHR::addFile(): Failed","The addFile() call failed to complete.");}
            }
        }

        function omitPolicyLoading() {
            updateFromPublicAPI();
            if (typeof arguments[0]===UNDEF) { doError(_flxhr.CALL_ERROR,"flXHR::omitPolicyLoading(): Failed","The omitPolicyLoading() call requires a boolean value"); }
            else if (!_error) {	// the SWF will actually catch if the open() hasn't yet been called (ie, readyState = 0), or if the send has already been called
                var a0=(typeof arguments[0]!==UNDEF)?arguments[0]:EMPTY;
                try { proxyObj.omitPolicyLoading(a0); }
                catch (err) { doError(_flxhr.CALL_ERROR,"flXHR::omitPolicyLoading(): Failed","The omitPolicyLoading() call failed to complete.");}
            }
        }

        function useMultipartLoader() {
            updateFromPublicAPI();
            if (typeof arguments[0]===UNDEF) { doError(_flxhr.CALL_ERROR,"flXHR::useMultipartLoader(): Failed","The useMultipartLoader() call requires a boolean value"); }
            else if (!_error) {	// the SWF will actually catch if the open() hasn't yet been called (ie, readyState = 0), or if the send has already been called
                var a0=(typeof arguments[0]!==UNDEF)?arguments[0]:EMPTY;
                try { proxyObj.useMultipartLoader(a0); }
                catch (err) { doError(_flxhr.CALL_ERROR,"flXHR::useMultipartLoader(): Failed","The useMultipartLoader() call failed to complete.");}
            }
        }
        ...
        publicAPI = {
            ...
            setParam:function() {
                updateFromPublicAPI();
                if (!queue_empty || readyState < 0) {
                    addToQueue(setParam,"setParam",arguments);
                    return;
                }
                setParam.apply({},arguments);
            },
            addFile:function() {
                updateFromPublicAPI();
                if (!queue_empty || readyState < 0) {
                    addToQueue(addFile,"addFile",arguments);
                    return;
                }
                addFile.apply({},arguments);
            },
            omitPolicyLoading:function() {
                updateFromPublicAPI();
                if (!queue_empty || readyState < 0) {
                    addToQueue(omitPolicyLoading,"omitPolicyLoading",arguments);
                    return;
                }
                omitPolicyLoading.apply({},arguments);
            },
            useMultipartLoader:function() {
                updateFromPublicAPI();
                if (!queue_empty || readyState < 0) {
                    addToQueue(useMultipartLoader,"useMultipartLoader",arguments);
                    return;
                }
                useMultipartLoader.apply({},arguments);
            },
