...
/*
 ...
 By: Lukasz
 change: Added albumprinterbonusprint for cookieDomainPeriods check.
 (also updated the s_code_version from v20-H25 to v21_H25)
 */

if (!s_account) {var s_account="albumprinterdev2";}

var s=s_gi(s_account);
var s_code_version = 'v21-H25';
/************************** CONFIG SECTION **************************/
...
if (s_account == 'albumprinterukprod' || s_account == 'albumprinterbonusprint'){s.cookieDomainPeriods="3";}
s.trackDownloadLinks=true;
s.trackExternalLinks=true;
s.trackInlineStats=true;
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx";
s.linkInternalFilters="javascript:,albelli.nl,albelli.fr,albelli.de,albelli.be,albelli.co.uk,albelli.it,albelli.es,albelli.eu,werkenbijalbelli.nl,albumprinter.com,filecatalogue.com,albelli.com,shop.albelli.com,onskefoto.se,allfoto.no,photoconnector.net,.albelli.,.photoconnector.,bonusprint.co.uk,file:///";
...
/* Time parting setup: Change below if EU law on DST start/end dates changes */
...
/* channelmanager settigs */
s._extraSearchEngines="bing.com|q|Microsoft Bing";
s._channelDomain="Social|facebook.com,twitter.com,hyves.nl,linkedin.com,myspace.com,msn.com,msnbc.com,msnbc.msn.com,t.co,blog.albelli.nl,m.facebook.com,symbaloo.com,flickr.com,ekudos.nl,nujij.nl,grubb.nl,netlog.com,plus.google.com";
s._channelParameter="";
s._channelPattern="Affiliates|affiliates_>CRM|crm_>Display|display_>Mailing|mailing_>Partner|partner_>SEA|sea_,ga_,GA_";
...
function s_doPlugins(s) {
    ...
    s.tnt = s.trackTNT();
    /* SiteCatalyst to TnT */
    var useOneOnOne=false; // switch to turn on/of the siteCatalyst mbox parameters
    if ((typeof mboxLoadSCPlugin == "function") && (useOneOnOne)) { // if mbox.js is loaded and the switch is on
        mboxLoadSCPlugin(s); // additional mbox call to TnT to track the SiteCatalyst variables in TnT.
    }

    /* Make sure s.events exists - before doing checks on it (to prevent errors occurring) */
    s.events=s.events?s.events:'';

    /* At the start of each visit set event 87*/
    var v_start = s.getVisitStart('s_visitStart');
    if (v_start == 1)
    { s.events=s.apl(s.events, 'event87', ',', 1); }

    /* Get the visitor status new vs repeat visitors */
    s.prop31=s.getNewRepeat();
    s.prop31=s.prop31.toLowerCase();
    if (s.prop31) s.eVar31=s.prop31;

    /* Get the visit number for this visitor */
    s.prop32=s.getVisitNum();
    if (s.prop32) s.eVar32=s.prop32;

    /* Get the days since the previous visit */
    s.prop33=s.getDaysSinceLastVisit('s_lv');
    s.prop33=s.prop33.toLowerCase();
    if (s.prop33) s.eVar33=s.prop33;

    /* Get the time and day settings for this request */
    s.prop34 = s.eVar34 = s.getTimeParting('h','+1'); // set hour of day
    s.prop35 = s.eVar35 = s.getTimeParting('d','+1'); // set day of week
    s.prop36 = s.eVar36 = s.getTimeParting('w','+1'); // set weekend/weekday

    /* Get the campaign, channel, keyword, referrer and Marin information with the channelmanager plugin */
    s.channelManager('utm_campaign', ':', 'c_cmp');
    if (s._campaignID.indexOf('#') > -1) {s._campaignID = s._campaignID.substring(0,s._campaignID.indexOf('#'));}
    if(!s.campaign) {s.campaign=s._campaignID;}
    if (s._campaignID.indexOf("mkwid") != -1) {s.eVar44=s.eVar45=s._campaignID;}
    s.eVar16 = s._channel;
    // s.eVar17 = s._partner;
    s.eVar18 = s._keywords.toLowerCase();
    // s.eVar19 = s._referringDomain;
    if (s.eVar16 == 'Natural Search')
    {
        if ((s.eVar18.indexOf('albelli') > -1) || (s.eVar18.indexOf('nskefoto') > -1) || (s.eVar18.indexOf('allfoto') > -1) || (s.eVar18.indexOf('abelli') > -1) || (s.eVar18.indexOf('albeli') > -1) || (s.eVar18.indexOf('albelie') > -1) || (s.eVar18.indexOf('allbelli') > -1) || (s.eVar18.indexOf('albellie') > -1))
        {
            s.eVar16 = 'SEO - Branded';
            if(!s.campaign)
            { s.campaign = 'SEO - Branded:' + s._partner + ':' + s.eVar18; }
        }
        else
        {
            s.eVar16 = 'SEO - non Branded';
            if(!s.campaign)
            { s.campaign = 'SEO - non Branded:' + s._partner + ':' + s.eVar18; }
        }
    } else s.eVar16 = '';

    /* Count the number of campaigns responded to in the last 90 days */
    if (s.campaign)
    {
        s.eVar27=s.eVar28=s.eVar15=s.campaign;
        var CampCountStr = s.crossVisitParticipation('1','sc_cvp_v30','90','20','>','purchase',1);
        if (CampCountStr)
        {
            var CampCountArr=CampCountStr.split('>');
            s.eVar30 = "" + CampCountArr.length;
        }
    }

    /* Stack the campaigns from the last 90 days, with a maxium of the latest 10 */
    s.eVar29=s.crossVisitParticipation(s.campaign,'sc_cvp_v29','90','10','>','purchase');

    /* Extract query string parameters, and store them in a variable, if getValOnce is used consecutive identical values will be deduped */
    if(!s.eVar2)
    {
        s.eVar2=s.getQueryParam('INID');
        if (s.eVar2.indexOf('#') > -1)
        { s.eVar2 = s.eVar2.substring(0,s.eVar2.indexOf('#')); }
        s.eVar2=s.getValOnce(s.eVar2,'c_intcmp',0);
    }

    if(!s.eVar21)
    {
        s.eVar21=s.getQueryParam('int_promo');
        if (s.eVar21.indexOf('#') > -1)
        { s.eVar21 = s.eVar21.substring(0,s.eVar21.indexOf('#')); }
    }

    if(!s.eVar23)
    {
        s.eVar23=s.getQueryParam('cross_sale_promo');
        if (s.eVar23.indexOf('#') > -1)
        { s.eVar23 = s.eVar23.substring(0,s.eVar23.indexOf('#')); }
        s.eVar23=s.getValOnce(s.eVar23,"c_cross_sale",1);
    }
    ...
    if(!s.eVar24)
    {
        s.eVar24=s.getQueryParam('we_promo');
        if (s.eVar24.indexOf('#') > -1)
        { s.eVar24 = s.eVar24.substring(0,s.eVar24.indexOf('#')); }
    }

    if(!s.eVar25)
    {
        s.eVar25=s.getQueryParam('vistaprint');
        if (s.eVar25.indexOf('#') > -1)
        { s.eVar25 = s.eVar25.substring(0,s.eVar25.indexOf('#')); }
    }

    if(!s.eVar26)
    {
        s.eVar26=s.getQueryParam('shopperid');
        if (s.eVar26.indexOf('#') > -1)
        { s.eVar26 = s.eVar26.substring(0,s.eVar26.indexOf('#')); }
    }

    if(!s.eVar40)
    {
        s.eVar40=s.getQueryParam('id');
        if (s.eVar40.indexOf('#') > -1)
        { s.eVar40 = s.eVar40.substring(0,s.eVar40.indexOf('#')); }
    }

    if(!s.eVar63) {
        s.eVar63=s.getQueryParam('search');
        if (s.eVar63.indexOf('#') > -1) {
            s.eVar63 = s.eVar63.substring(0,s.eVar63.indexOf('#'));
        }
        if(s.eVar63) {
            s.eVar63=s.eVar63.toLowerCase()
            s.eVar63=s.getValOnce(s.eVar63,"c_search",1);
        }
    }
    if(s.eVar63) {
    ...
    /* Code to count specific events only once during a certain timeframe */
    var evt;
    var newEvents = "";
    // List the events which have to submitted one time in X days >  eventY:  X
    var dailyEventList = { event4: 1, event13: 1, event15: 1, event16: 1, event19: 1, event20: 1, event21: 1, event22: 1, event23: 1, event24: 1, event25: 1,  event26: 1,  event27: 1, event28: 1, event29: 1, event30: 1, event44: 1, event47: 1,  event50: 1,  event51: 1,  event52: 1,  event53: 1,  event54: 1,  event55: 1,  event66: 1,  event74: 1, event76: 1, event77: 1  };
    if ( s.events != "" )
    {
        var evtArray = s.events.split( "," );
        for ( i=0; i<evtArray.length; i++)
        {
            evt = evtArray[i];
            // Test if the current events in s.events exists in daylyEventList
            if ( dailyEventList[evt] )
            {
                // return event if not existing,or empty is already defined
                evt = s.getValOnceDay( evt, "s_" + evt, dailyEventList[evt] );
            }

            // rebuild the s.events variable
            if ( evt != "" )
            {
    ...
    /**
     * Merchandising slots.
     * Lists the merchandising slot links on the page, if the WordPress plugin Merchandising Slots is enabled.
     */
    if (typeof MerchandisingSlots === 'function' ) {
        (function () {
            var assetList = new MerchandisingSlots();
            if (assetList.intPromoAssetInfoList != '') {
                if (s.events == '') {
                    s.events = 'event2';
                } else {
                    s.events += ',event2';
                }
                s.list1 = assetList.intPromoAssetInfoList;
            }
        })();
    }
    ...
    /* Copy of the purchase ID voor weergave in de reporting */
    if(s.purchaseID) {s.eVar7=s.purchaseID;}
    { s.prop11 = s_code_version; }

    /* Plugin for VWO */
    try {
        if (typeof(_vis_opt_settings_loaded) == "boolean")
        {
            var _combination = _vis_opt_readCookie('_vis_opt_exp_'+_vis_opt_experiment_id+'_combi') || _vwo_exp[_vis_opt_experiment_id].combination_chosen;
    ...
    /* Hierarchy1 setting from pageName */
    jh_pageName = s.pageName;
    jh_pageName = jh_pageName.split("/").join(":");
    jh_pageName = jh_pageName.replace(' : :',':');
    jh_pageName = jh_pageName.replace(': :',':');
    jh_pageName = jh_pageName.replace('::',':');
    jh_pageName = jh_pageName.replace(' : ',':');
    s.hier1=jh_pageName
    ...
    /* Copy variables */
    s.eVar41    = "D=pageName";
    s.prop16    = window.location.host;
    s.prop40		= "D=v63";
    s.channel   = "D=c4";

    /* Check if purchaseID is available to set the order and order pending event */
    if(!s.purchaseID)
    {
    ...
    /**
     * TRACKING MOBILE TEMPLATE AND STATES
     *
     * We will check the viewport state and track it initial state.
     */
    // Check if the Theme Type code is defined
    if (!(typeof s.themetype === 'undefined') && s.prop39===undefined)
    {

        // Set initial state of mobileTracking vars
        initialviewportState = getViewPortState();

        // If it's defined, we need to chek if it's responsive or not
        if (s.themetype=="Responsive")
        {
            // Set final value to prop39 width TYPE|STATE
            s.prop39=s.themetype+"|"+initialviewportState;
        }
        else
        {
            // Set final value to prop39 width TYPE|STATE
            s.prop39=s.themetype;
        }

        console.log("s.prop39: "+s.prop39);
    }
...
// Flag that indicates if the tracking info should be logged into the console
mobileTrackingConsoleLog=true;

// Array with Mobile Tracking values
mobileTracking = new Array();
mobileTracking['viewportStateChangeMaxTrack']=2;
mobileTracking['viewportStateChangeTrack']=0;


// Tracking functions to be executed when the page is loaded
window.onload = function() {

    // Set initial state of mobileTracking vars
    mobileTracking['viewportState'] = getViewPortState();
}


// Tracking functions to be executed when the page is resized
window.onresize = function() {

    // delay actions to only be triggered after 0.5 of resize inactivity
    // this way they are only executed after the user stopped the resize
    // and not multiple times during the resize process
    delayWindowResize(function()
    {
        /**
         * TRACKING STATE SWITCH
         *
         * This has to be triggered on an event of viewport state change.
         * We will check, on window resize, if the new state is diferent from the actual one
         * and track this change.
         * This state switch tracking will be done a maximum of "mobileTracking['viewportStateChangeMaxTrack']"
         * times per pageview.
         */

        // check if we already reached the maximum trackable rezise changes
        if (mobileTracking['viewportStateChangeTrack'] < mobileTracking['viewportStateChangeMaxTrack'])
        {
            // get the new viewport state
            newViewportState = getViewPortState();

            // if it's diferent from the previous one, then we will track it
            if (newViewportState!=mobileTracking['viewportState'] && mobileTracking['viewportState']!==undefined)
            {
                // add tracking event
                s.events = "event3";
                s.linkTrackVars = "events";
                s.linkTrackEvents = "event3";
                s.tl (this, 'o', 'Discovery: Responsive state: '+mobileTracking['viewportState']+'-'+newViewportState);

                // increase the viewportStateChangeTrack counter
                mobileTracking['viewportStateChangeTrack']++;

                // log on console for debug purposes
                if (mobileTrackingConsoleLog)
                { console.log(mobileTracking['viewportStateChangeTrack']+' | Discovery: Responsive state: '+mobileTracking['viewportState']+'-'+newViewportState); }

                // update the viewportState
                mobileTracking['viewportState']=newViewportState;
            }
            else
            {
                // update the viewportState
...
/**
 * Returns the viewport state, according to pre-defined breakpoints.
 * Available viewport states are:
 * - Large: >1024
 * - Medium: 640>1024
 * - Small: <640
 *
...
function getViewPortState()
{
    ...
    // Depending on the viewport width, we need to set the state to
...
/**
 * Delays window.onresize actions to only be triggered after a certain time.
 */
var delayWindowResize = (function(){
    var timer = 0;
 ...
 * Plugin: getValOnceDay  - get a value once per session or number of days (end of current Day)
 */
s.getValOnceDay = function (v,c,e) {
    var s=this;
    k=s.c_r(c);
    a=new Date;
    e=e?e:0;
...
s.visitorNamespace="albumprinter"
s.trackingServer="albumprinter.122.2o7.net"
